...
- [1. Overview](#1-overview)
  - [1.1. The parse-numeric Node.js module](#11-the-parse-numeric-nodejs-module)
...
- [3. Installing parse-numeric](#3-installing-parse-numeric)
...
### 1.1. The **parse-numeric** Node.js module

**parse-numeric** is a Node.js [module
...
**parse-numeric** revisions as [packages
...
**parse-numeric** uses [`semantic-release`][semantic-release-readme]. Whenever
source code is merged into `master` (after passing all unit tests with code
coverage thresholds satisfied), we automatically:

1.  **Bump the semantic version** of **parse-numeric** (based on conventional
    commits).
...
4.  **Publish** the **parse-numeric** to the public npm registry.
...
Open a Terminal and change into your **parse-numeric** directory.
...
```shell
> parse-numeric@0.0.0 semantic-release /Users/username/Projects/github/gregswindle/parse-numeric
...
## 3. Installing parse-numeric

```shell
npm install **parse-numeric** --global
```
...
The only real exception is if **parse-numeric** is completely unusable by most
