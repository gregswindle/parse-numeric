...
> This guide is for those who want to contribute code to **parse-numeric**.
> This guide describes how to set up your development environment so that you
> can build and test **parse-numeric**.
...
In order to work with **parse-numeric** as a developer, we recommend you feel
comfortable with:

| Skill                                                     | Reason                                                                                                     |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| ![code][octicon-code]&nbsp;JavaScript                     | **parse-numeric** is written in JavaScript (CommonJS)                                                      |
...
| ![platform][octicon-server]&nbsp;Node.js                  | **parse-numeric** runs on Node.js                                                                          |
| ![terminal][octicon-terminal]&nbsp;Command-line-interface | We use the CLI to build, test, and publish **parse-numeric**                                               |
...
Before you can build and test **parse-numeric**, you must install and configure
...
1.  [Git ![link-external][octicon-link-external]](http://git-scm.com)
    ...
    > **parse-numeric** is hosted on github and uses Git for source control. In
...
2.  [Node.js ![link-external][octicon-link-external]](http://nodejs.org),
    ...
    > **parse-numeric** uses Node.js modules to generate tables of contents,
...
3.  **NPM** (which installs with Node.js) or
    ...
    > NPM and Yarn install and update `parse-numeric's` third-party
    > dependencies.
...
2.  [**Fork**](https://docs.github.com/ee/github-basics/fork-project.html#how-to-fork-a-project)
    the
    [main **parse-numeric** repository](https://github.com/gregswindle/parse-numeric/forks/new)
    (aka, "`origin`").
3.  **Clone _your_ fork** of the **parse-numeric** repository and define an
    `upstream` remote pointing back to the **parse-numeric** repository that you
...
<details><summary><img src="https://cdnjs.cloudflare.com/ajax/libs/octicons/8.1.3/svg/package.svg" height="30" width="30" valign="bottom"> <strong>Toggle **parse-numeric**  view...</strong></summary>
...
to user <samp>parse-numeric's</samp> CLI (Command-Line Interface).
...
1. Create a global symbolic link to **parse-numeric**:

   ```shell
   npm link
   ```

1. Now you can test and use **parse-numeric** with two (2) CLI commands:
...
> [![Jest BDD][jest-logo] **parse-numeric** uses Jest for Behavior-Driven
...
![verified][octicon-verified] **parse-numeric** uses
...
**parse-numeric** is a Node.js module. **parse-numeric** is published to npm,
...
![**parse-numeric** component diagram](../img/dldp-components.png)

## 10. Roadmap

> ![Telesope][octicon-telescope] View the
> [ROADMAP for **parse-numeric**](../../ROADMAP.md).
...
[issue-workflow-img]:
  https://github.com/gregswindle/parse-numeric/raw/master/docs/icons8/contribution-lifecycle-create-issue.png
[label-status-triage-img]:
  https://fakeimg.pl/200x24/d4c5f9/000/?text=status:+triage&font_size=20&font=museo
[pr-workflow-img]:
  https://github.com/gregswindle/parse-numeric/raw/master/docs/icons8/contribution-lifecycle-pr.png
